...
class QAgent(object):
    ...
    def train(self):
        ...
        loss1 = self.layer_q_estimator.update(state_batch, action1_batch, target1_batch)
        ...
        state_batch = np.array(state_batch)
        loss2 = self.width_q_estimator.update(state_batch, action2_batch, target2_batch)
    ...
    def feed_memory(self, state, action1,action2, reward, next_state,done):
        self.layer_memory.save(self.normalizer.normalize(state), action1, reward, self.normalizer.normalize(next_state), done)
